...
app.post('/api/validate', (req, res) => {
  ...
  const approvedActivations = keyObj.activations.filter(a => a.status === 'approved');
  const alreadyApprovedHere = approvedActivations.some(a => a.device_id === deviceId);
  const maxAllowedDevices = keyObj.max_devices || 2;
  if (!alreadyApprovedHere && approvedActivations.length >= maxAllowedDevices) {
    return res.json({ 
      valid: false, 
      needs_approval: false, 
      message: `⚠️ هذا الكود مستخدم بالفعل على الحد الأقصى للأجهزة (${maxAllowedDevices} أجهزة)!` 
    });
  }
