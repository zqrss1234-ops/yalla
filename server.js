...
// ==========================================
// 🛡️ طبقة التخزين السحابي والمحلي
// ==========================================
let memoryDB = { keys: INITIAL_KEYS, adminToken: ADMIN_TOKEN };
...
async function initMongoCloud() {
  ...
  try {
    ...
    if (remoteDoc && Array.isArray(remoteDoc.keys) && remoteDoc.keys.length > 0) {
      const existingKeys = new Set(remoteDoc.keys.map(k => k.key));
      let addedCount = 0;
      for (const ik of INITIAL_KEYS) {
        if (!existingKeys.has(ik.key)) {
          remoteDoc.keys.push(ik);
          addedCount++;
        }
      }
      memoryDB = { keys: remoteDoc.keys, adminToken: remoteDoc.adminToken || ADMIN_TOKEN };
      saveLocalDB(memoryDB);
      if (addedCount > 0) {
        await mongoCollection.updateOne(
          { _id: 'master_license_store' },
          { $set: { keys: memoryDB.keys, updatedAt: new Date().toISOString() } }
        );
      }
    } else {
      await mongoCollection.updateOne(
        { _id: 'master_license_store' },
        { $set: { _id: 'master_license_store', keys: memoryDB.keys, adminToken: ADMIN_TOKEN, updatedAt: new Date().toISOString() } },
        { upsert: true }
      );
    }
  } catch (err) {
...
async function loadData() {
  try {
    ...
    // 🛡️ الحفظ الذاتي التلقائي (Auto-Vault Sync): مزامنة فورية تمنع ضياع أي كود حتى لو نام السيرفر
    const cachedVault = localStorage.getItem('ys_persistent_vault');
    if (cachedVault) {
      try {
    ...
    localStorage.setItem('ys_persistent_vault', JSON.stringify(allKeys));
